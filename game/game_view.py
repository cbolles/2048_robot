...
class TileView:
    ...
    def get_image(self, value):
        ...
        image_path = str(resource_dir / 'tiles' / image_name)
...
class GameView:
    ...
    def draw_discard_pile(self, game_model):
        ...
        box_height = self.discard_pile_dim[1] / game_model.discard_pile.max_discards
    ...
    def draw_tile_queue(self, game_model):
        ...
        for i in range(len(game_model.tile_queue.tile_values) - 1, -1, -1):
            tile_value = game_model.tile_queue.tile_values[i]
            self.tile_views[tile_value].draw(self.screen, tile_x, tile_y)
            tile_x += tile_delta
